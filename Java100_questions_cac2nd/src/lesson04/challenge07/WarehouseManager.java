...
public class WarehouseManager {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < ABKosanArray.length; i++) {
			for (int j = i + 1; j < ABKosanArray.length; j++) {
				if (ABKosanArray[i] > ABKosanArray[j]) {
					change_num = ABKosanArray[i];
					ABKosanArray[i] = ABKosanArray[j];
					ABKosanArray[j] = change_num;
				}
			}
		}
		...
		//ここに降順にソートする処理を記述する
		for (int i = 0; i < ABKosanArray.length / 2; i++) {
			change_num = ABKosanArray[i];
			ABKosanArray[i] = ABKosanArray[ABKosanArray.length - 1 - i];
			ABKosanArray[ABKosanArray.length - 1 - i] = change_num;
		}
